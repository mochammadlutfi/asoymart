...
class StokAwalController extends Controller
{
     ...
     * @return void
     */
    protected $produkUtil;
    private $produkRepo;
    public function __construct(ProdukUtil $produkUtil, ProdukRepository $produkRepo)
    {
    ...
    public function index($produk_id)
    {
        ...
        ->where('id', $produk_id)->first();
        $variasi = ProdukVariasi::where('produk_id', $produk_id)->where('kelola_stok', 1)->get();

        // $toko_id = request()->session()->get('user.toko_id');

        $transaksi = Transaksi::where('bisnis_id', $bisnis_id)
        ->where('outlet_id', $outlet_id)
        ->where('stok_awal_produk_id', $produk_id)
        ...
        return view('mitra.inventaris.stok_awal')
                    ->with(compact(
                        'produk',
                        'variasi',
                        'pembelian',
