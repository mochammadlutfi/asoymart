...
class VariasiProdukController extends Controller
{
        ...
        if ($validator->fails()){
            ...
            if($request->has('old_sku'))
            {
                ...
                if($request->old_kelola_stok == '1')
                {
                    ...
                    $variasi[0]['satuan_nama'] = $this->satuanNama($request->old_satuan_id);
                ...
                if($kelola_stok == 1)
                {
                    ...
                    $variasi[1]['satuan_nama'] = $this->satuanNama($request->variasi_unit);
                ...
                if($kelola_stok === 1)
                {
                    ...
                    $variasi[0]['satuan_nama'] = $this->satuanNama($request->variasi_unit);
            ...
            if($row_count > 1)
            {
                return view('mitra.produk.include.entry_variasi', compact('variasi','row_count'));
            }else{
                return view('mitra.produk.include.tbl_variasi', compact('variasi','row_count'));
            }
    ...
    public function jsonModal(Request $request)
    {
        ...
        foreach($produkVariasi as $pv)
        {
            ...
            $variasi[] = view('mitra.produk.include.modal_variasi_entry', compact('pv', 'i', 'hrg', 'tipe'))->render();
            $i++;
        }
